...
export function FloatingWhatsApp({ phoneNumber, agencyName }: FloatingWhatsAppProps) {
    if (!phoneNumber) return null;

    // Clean phone number (remove non-digits, ensure country code)
    const cleanNumber = phoneNumber.replace(/\D/g, "");
    // Default message
    const text = encodeURIComponent(`Hi ${agencyName}, I'm interested in the route on PathSathi.`);
    const href = `https://wa.me/${cleanNumber}?text=${text}`;
